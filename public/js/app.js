...
socket.on('inCheck', turn => {
    if (board.orientation().includes(turn)) {
        statusEl.textContent = "Estás en Jaque!!"
    }
    else {
        statusEl.textContent = "La oponente está en jaque !!"
    }
})
...
socket.on('gameOver', (turn, win) => {
    ...
    if (win) {
        if (board.orientation().includes(turn)) {
            statusEl.textContent = "Perdiste, mejor suerte la próxima vez :)"
        }
        else {
            statusEl.textContent = "¡¡Felicitaciones has ganado!!"
        }
    }
    else {
        statusEl.value = 'Sorteo del juego'
    }
})

//Client disconnected in between
socket.on('disconnectedStatus', () => {
    alert('¡El oponente salio del juego!')
    messageEl.textContent = '¡El oponente salio del juego!'
})
...
joinButtonEl.addEventListener('click', (e) => {
    ...
    if (!user || !room) {
        messageEl.textContent = "¡Los campos de entrada no pueden estar vacíos!"
    }
    else {
        ...
        messageEl.textContent = "Esperando a que otro jugador se una"
    }
})
